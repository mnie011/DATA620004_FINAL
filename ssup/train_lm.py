...
from vae import ResNet18
from utils import progress_bar, cutmix_data, cutout_data, mixup_data
from loss_utils import CECriterion, CutmixCriterion
from torch.autograd import Variable


def parse_config():
    parser = argparse.ArgumentParser(description='arg parser')
    parser.add_argument('--cfg_file', type=str, default='../cfgs/ssup/lm-r18.yaml', help='specify the config for training')
    parser.add_argument('--model_name', type=str, default='R18')
    ...
    parser.add_argument('--pretrain_ckpt', type=float ,default=None, required=False)
...
def main():
    ...
    if args.pretrain_ckpt is None:
        args.pretrain_ckpt = cfg.PRETRAIN_CKPT
    ...
    if args.model_name == 'R18':
        net = ResNet18(100, args.pretrain_ckpt)
    else:
    ...
    for epoch in range(start_epoch, epochs):
        ...
        writer.add_scalar('ACC_test_{}'.format(args.version), acc, epoch)  # tensorboard acc
        if acc > best_acc:
            print('Saving..')
            state = {
                'net': net.state_dict(),
            ...
            torch.save(state, '../muse/checkpoints/{}_{}.pth'.format(args.model_name, args.version))
            best_acc = acc
